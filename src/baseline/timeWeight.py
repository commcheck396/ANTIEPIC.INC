...
def get_user_time_span(user_data_map, user_id, game_id):
    """
    获取评论时间和现在的时间间隔
    """
    if user_id not in user_data_map:
...
#
#         reviewDays = get_user_time_span(user_data_map, user_id, game_id)
#         if reviewDays is None:
#             print(user_id, game_id)
#             time_weight.append(0)
#             continue
#         time_weight.append(np.exp(-reviewDays / 365))
for user_id, user_data in user_data_map.items():
...
X_train, X_test, y_train, y_test, weights_train, weights_test = train_test_split(X, y, time_weight, test_size=0.2,
                                                                                 random_state=42)
